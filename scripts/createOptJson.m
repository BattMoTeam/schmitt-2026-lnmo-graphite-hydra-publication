function [jsonOpt, filename] = createOptJson(filename)

    if nargin == 0
        filename = 'IMP5-70-120-H0B_graphite-lnmo_schmitt-2026_validation.battmo.json';
    end

    fnEQC         = fullfile(getHydra0Dir(), 'parameters', 'equilibrium-calibration-parameters.json');
    jsonstructEC  = parseBattmoJson(fnEQC);
    fnHRC         = fullfile(getHydra0Dir(), 'parameters', 'high-rate-calibration-parameters.json');
    jsonstructHRC = parseBattmoJson(fnHRC);
    ...
    % Compare
    fnBase = fullfile(getHydra0Dir(), 'parameters', 'h0b-base.json');
    jsonBase = parseBattmoJson(fnBase);
    ...
    % Effective conductivities are set in runHydra
    ...
    dosave = true;
    if dosave
        fullfilename = fullfile(getHydra0Dir(), 'parameters', filename);
        writeStruct(jsonOpt, fullfilename);
    end
