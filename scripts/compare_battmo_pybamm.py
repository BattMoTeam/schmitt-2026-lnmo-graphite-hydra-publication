...
def compute_metrics(reference_time, reference_voltage, comparison_time, comparison_voltage) -> dict:
    ...
    reference_time, reference_voltage, comparison_time, comparison_voltage = (
        ...
        for values in (reference_time, reference_voltage, comparison_time, comparison_voltage)
    )
    ...
    time_interp = np.unique(np.concatenate(([start, end], reference_time, comparison_time)))
    ...
    return {
        ...
        "battmo_overlap_fraction": float(duration / (reference_time[-1] - reference_time[0])),
        "rmse_v": rmse_v,
...
def run_pybamm_case(
    ...
    *,
    ...
    time_grid = np.linspace(0, duration_s, max(2, int(np.ceil(duration_s / output_period_s)) + 1))
...
def make_plot(cases: list[dict], output_path: Path) -> None:
    ...
    for ax, case in zip(axes.flat, cases):
        for key, label, color, style in (
            ...
            ax.plot(capacity, curve["voltage_v"], color=color, linestyle=style, label=label)
...
def compare(
    ...
    reference_json, bpx_path, summary_path, figure_path = (
        Path(path).resolve() for path in (reference_json, bpx_path, summary_path, figure_path)
    )
    ...
    for case in reference["cases"]:
        result = run_pybamm_case(
            parameter_values, float(case["current_a"]), float(case["experimental"]["time_s"][-1])
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--reference", type=Path, default=FIGURES_DIR / "battmo-validation-reference.json"
    )
    ...
    parser.add_argument(
        "--figure-output", type=Path, default=CODEX_FIGURES_DIR / "battmo-vs-pybamm-bpx.png"
    )
