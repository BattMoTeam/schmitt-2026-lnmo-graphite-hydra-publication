...
for plotIndex = 1:numel(sensitivityFields)
    ...
    if dosave && plotIndex == 3
        % only save relative Sens
        saveFigureSet(sensitivityFigure, fullfile(getHydra0Dir(), 'figures', figureFilenames{plotIndex}));
    end
end

if dosave
    figureData = struct('parameters', {cellstr(sensitivityTable.Parameter)}, ...
        'log10_absolute_relative_sensitivity', log10(abs(sensitivityTable.relativeSensitivity))');
    writeStruct(figureData, fullfile(getHydra0Dir(), 'figures', ...
        'figure-15-relative-parameter-sensitivities.json'));
end
