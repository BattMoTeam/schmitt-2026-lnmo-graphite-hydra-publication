...
% Plot the eigenvectors of a Hessian as a heatmap.
    ...
    opt = struct('dosave', false, ...
                 'filename', 'figure-16-hessian-eigendecomposition', ...
                 'titlestr', sprintf('Eigenvectors of %s Hessian', casename));
    ...
    if opt.dosave
        ...
        saveFigureSet(fig, fullfile(figdir, opt.filename));
        figureData = struct('parameters', {shortnames}, 'method', casename, ...
            'eigenvalues', eigenvals(:)', 'eigenvectors', eigenvecs);
        writeStruct(figureData, fullfile(figdir, [opt.filename, '.json']));
    end
