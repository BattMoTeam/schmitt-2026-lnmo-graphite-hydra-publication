...
Figures are saved in the figures folder. High-rate calibration also
saves Hessian results in a run-specific subfolder beside
...
`runValidation` also saves `battmo-validation-reference.json`
...
and reports scaled, physical, and relative parameter
sensitivities. This is used in `plotSensitivities`.

In the validation,`createOptJson` (`scripts/createOptJson.m`) is
called to export the merged calibrated parameters into
...
For Python validation, run
```
python run-validation.py
```
after `runReproduction` (typical duration: `< 1 min`). It exports BPX and
JSON-LD and runs the PyBaMM comparison.

The times are rough wall-clock estimates on a typical computer. The
high-rate calibration dominates runtime and can vary substantially
with your CPU.
...
- `IMP5-70-120-H0B_graphite-lnmo_schmitt-2026_validation.battmo.json`:
  the merged calibrated parameters.

The recursive merge applies base, equilibrium, then high-rate
parameters, matching `runValidation.m`. Current collectors and
regional electrolyte Bruggeman coefficients are enabled. Geometry
files remain separate and is included by `runHydra` (the main
backend simulation script).

The merged BattMo parameter file can be converted to JSON-LD with:

```sh
python scripts/convert_from_battmo_to_jsonld.py
```
(this is also done when running the `run-validation.py` script.

These standalone scripts reads only
`parameters/IMP5-70-120-H0B_graphite-lnmo_schmitt-2026_validation.battmo.json`
and writes
`linked-data/IMP5-70-120-H0B_graphite-lnmo_schmitt-2026_validation.battmo.jsonld`.
The JSON-LD output uses BattINFO/EMMO component and quantity mappings,
with an embedded context. Model-specific settings use a separate
project namespace; the complete source JSON, including function
definitions, is retained in `sourceData`.  All parameter values come
from the merged input file; the converter does not reread or merge the
base and calibration files.
...
Running `runReproduction` generates the figures used in the
publication. These can be studied interactively in the
documentation. The publication explorer includes Figures 12–16 as
interactive plots and PNG exports. The calibration, validation, and
sensitivity scripts also export their interactive JSON data alongside
the images in `figures/` for use in the interactive
documentation. Building the site uses the saved outputs and does not
rerun calibration.

The Python summaries, FAIR exports, and site assets can be refreshed
by:
...
file has not yet been updated (this is done in
`runValidation`). Documentation dependencies are listed in
[Dependencies](#dependencies).  The generated site is in `site/`;
`python -m mkdocs serve` provides a local preview.

The browser fetches the JSON files directly and caches each response
for reuse. Use the local preview server `mkdocs serve` or GitHub
Pages. Opening `site/index.html` directly with `file://` will not
allow the browser to fetch the data files.

## Dependencies

The BattMo calibration, PyBaMM interoperability, and documentation
workflows have separate dependencies so you can install the software
needed for the stages you intend to run.

| Dependency | Why it is needed |
| --- | --- |
| Git with GitHub access | Clones this repository and the submodules using their configured URLs. |
| MATLAB | Runs BattMo: calibration, validation, sensitivity analysis, and publication figure generation. |
| Python | Runs JSON-LD/BPX conversion and the PyBaMM comparison and plotting scripts. The interoperability workflow has been checked with Python 3.12. |
...
| --- | --- |
| `pybamm[plot,bpx]` | Imports the BPX parameter validator and runs the DFN simulations in PyBaMM for comparison with BattMo. |
