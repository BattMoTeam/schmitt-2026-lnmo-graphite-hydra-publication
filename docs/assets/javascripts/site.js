...
async function loadDataEntry(entry) {
  const key = `${entry.format || "json"}:${entry.path}`;
  if (!dataRequests.has(key)) {
    const request = fetch(entry.path).then((response) => {
      if (!response.ok) {
        throw new Error(`Failed to load ${entry.path}: HTTP ${response.status}`);
      }
      return entry.format === "text" ? response.text() : response.json();
    }).catch((error) => {
...
function loadManifest() {
  return loadDataEntry({ path: "assets/data/site-manifest.json" });
}
...
async function initValidationExplorer(manifest) {
  ...
  const reference = await loadDataEntry(manifest.data.validation_reference);
  ...
  // Evaluate errors at experimental sample times, matching the displayed reference curves.
  const rows = reference.cases.map((entry) => {
    const simulation = entry.battmo;
    const experiment = entry.experimental;
    const commonEnd = Math.min(experiment.time_s.at(-1), simulation.time_s.at(-1));
    const differences = [];
    let left = 0;
    for (let index = 0; index < experiment.time_s.length; index += 1) {
      const time = experiment.time_s[index];
      if (time > commonEnd) break;
      while (left + 1 < simulation.time_s.length && simulation.time_s[left + 1] <= time) {
        left += 1;
      }
      let voltage = simulation.voltage_v[left];
      // Clamp before the first simulated point, as in the original numpy interpolation.
      if (time > simulation.time_s[left] && left + 1 < simulation.time_s.length) {
        const fraction = (time - simulation.time_s[left]) /
          (simulation.time_s[left + 1] - simulation.time_s[left]);
        voltage += fraction * (simulation.voltage_v[left + 1] - voltage);
      }
      differences.push((voltage - experiment.voltage_v[index]) * 1000);
    }
    return {
      case: entry.case_name,
      current_A: entry.current_a,
      rmse_mV: Math.sqrt(differences.reduce((sum, value) => sum + value * value, 0) / differences.length),
      mae_mV: differences.reduce((sum, value) => sum + Math.abs(value), 0) / differences.length,
      max_abs_mV: differences.reduce((maximum, value) => Math.max(maximum, Math.abs(value)), 0),
      final_diff_mV: differences.at(-1),
    };
  });
  renderTable(tableDiv, rows);
}
...
async function initPublicationFigures(manifest) {
  ...
  const fig15Plot = byId("figure15-plot");
  const fig16Plot = byId("figure16-plot");
  const gallery = byId("publication-gallery");

  if (!fig12Discharge && !fig12Ocv && !fig13Plot && !fig14Plot && !fig15Plot && !fig16Plot && !gallery) {
    return;
  }

  const Plotly = ensurePlotly();
  const [fig12, fig13, fig14, fig15, fig16] = await Promise.all([
    loadDataEntry(manifest.data.figure12),
    loadDataEntry(manifest.data.figure13),
    loadDataEntry(manifest.data.figure14),
    loadDataEntry(manifest.data.figure15),
    loadDataEntry(manifest.data.figure16),
  ]);
  ...
  if (fig15Plot) {
    await Plotly.react(fig15Plot, [{
      type: "bar",
      x: fig15.parameters,
      y: fig15.log10_absolute_relative_sensitivity,
      hovertemplate: "%{x}<br>log10(abs(p*dJ/dp)) = %{y:.4f}<extra></extra>",
    }], withPlotHeight(fig15Plot, baseLayout(
      "Initial relative parameter sensitivities", "Parameter", "log10(abs(p*dJ/dp))", {
        xaxis: Object.assign(baseAxis("Parameter", getThemePalette()), { tickangle: -45 }),
        margin: { t: 72, r: 28, b: 130, l: 88 },
      },
    )), plotConfig());
  }

  if (fig16Plot) {
    const modes = fig16.eigenvalues.map((value, index) =>
      `Mode ${index + 1}<br>eigenvalue=${value.toPrecision(3)}`);
    await Plotly.react(fig16Plot, [{
      type: "heatmap",
      x: modes,
      y: fig16.parameters,
      z: fig16.eigenvectors,
      zmin: -1,
      zmax: 1,
      colorscale: [[0, "blue"], [0.5, "white"], [1, "red"]],
      texttemplate: "%{z:.2f}",
      hovertemplate: "%{y}<br>%{x}<br>component=%{z:.5f}<extra></extra>",
      colorbar: { title: { text: "Component", side: "right" } },
    }], withPlotHeight(fig16Plot, baseLayout(
      `${fig16.method} Hessian eigenvectors and eigenvalues`, "Eigenmode", "Scaled parameter", {
        yaxis: Object.assign(baseAxis("Scaled parameter", getThemePalette()), { autorange: "reversed" }),
        margin: { t: 72, r: 100, b: 130, l: 130 },
      },
    )), plotConfig());
  }
...
async function rerenderInteractiveContent() {
  const manifest = window.__DOCS_STATE__.manifest || await loadManifest();
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    installThemeToggle();
    applyThemeMode(getThemeMode(), false);
    window.__DOCS_STATE__.manifest = await loadManifest();
